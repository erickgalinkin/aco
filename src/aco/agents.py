...
class CardiffPPO(BaseAgent):
    def __init__(
        self,
        input_dims=52,
        action_space=[i for i in range(158)],
        lr=0.002,
        betas=[0.9, 0.990],
        gamma=0.99,
        K_epochs=4,
    ...
    def get_action(self, observation, action_space=None):
        ...
        action = self.old_policy.act(
            state, self.memory, deterministic=self.deterministic
        )
    ...
    def select_decoy(self, host, observation):
        try:
            ...
            action = [
                a
                for a in self.greedy_decoys[host]
                if a not in self.current_decoys[host]
            ][0]
    ...
    def train(self):
        ...
        old_logprobs = (
            torch.squeeze(torch.stack(self.memory.logprobs)).to(device).detach()
        )
    ...
    def set_initial_values(self, action_space, observation=None):
        ...
        self.policy = CardiffAC(self.input_dims, self.n_actions).to(device)
        if self.restore:
            pretained_model = torch.load(
                self.ckpt, map_location=lambda storage, loc: storage
            )
        ...
        self.old_policy = CardiffAC(self.input_dims, self.n_actions).to(device)
        self.old_policy.load_state_dict(self.policy.state_dict())
